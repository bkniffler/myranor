...
import { PropertyTypes, FacilityTypes } from '../../core';
...
export class ConsoleUI {
  ...
  private displayGameInfo(): void {
    ...
    if (player.propertyIds.length === 0) {
      ...
      player.propertyIds.forEach((id) => {
        const property = state.properties[id];
        if (property) {
          console.log(
            `  ${property.name} (${property.active ? 'aktiv' : 'inaktiv'})`
          );

          // Display facilities on this property
          if (property.facilityIds.length > 0) {
            property.facilityIds.forEach((facilityId) => {
  ...
  private showGainInfluenceAction(): void {
    ...
    this.rl.question('Wie viel Gold möchtest du ausgeben? (0 zum Abbrechen): ', (amountStr) => {
      ...
      if (result) {
        console.log(`Du hast ${amount} Gold ausgegeben und ${amount * 2} Einfluss gewonnen.`);
      } else {
  ...
  private showSellMaterialsAction(): void {
    ...
    console.log('Du kannst Rohmaterialien und Sondermaterialien verkaufen, um Gold zu gewinnen.');

    // Display available materials
    console.log('\nVerfügbare Rohmaterialien:');
    const rawMaterials: string[] = [];
    for (const [material, amount] of Object.entries(player.resources.rawMaterials)) {
      if (amount && amount > 0) {
        console.log(`  ${material}: ${amount}`);
    ...
    for (const [material, amount] of Object.entries(player.resources.specialMaterials)) {
      if (amount && amount > 0) {
        console.log(`  ${material}: ${amount}`);
    ...
    this.rl.question('\nWelches Material möchtest du verkaufen? ', (materialName) => {
      if (!materialName) {
        console.log('Aktion abgebrochen.');
        this.showActionMenu();
        return;
      ...
      if (rawMaterials.includes(materialName) && player.resources.rawMaterials[materialName]) {
        materialType = 'raw';
        materialAmount = player.resources.rawMaterials[materialName] || 0;
      } else if (specialMaterials.includes(materialName) && player.resources.specialMaterials[materialName]) {
      ...
      this.rl.question(`Wie viel ${materialName} möchtest du verkaufen? (Max: ${materialAmount}): `, (amountStr) => {
        const amount = parseInt(amountStr, 10);
        ...
        const result = this.engine.executeCommand({
          ...
          payload,
          validate: (state) => {
            return state.phase === 'action' && state.actionPointsRemaining > 0;
          },
          execute: (state) => {
            ...
            return [{
              ...
              playerId: state.currentPlayerId,
              payload: {
                rawMaterials: materialType === 'raw' ? { [materialName]: amount } : {},
                specialMaterials: materialType === 'special' ? { [materialName]: amount } : {},
                goldGained
              },
              apply: (state) => {
                const player = { ...state.players[state.currentPlayerId] };
                ...
                if (materialType === 'raw') {
                  ...
                  updatedRawMaterials[materialName] = (updatedRawMaterials[materialName] || 0) - amount;
                  player.resources.rawMaterials = updatedRawMaterials;
                } else {
                  const updatedSpecialMaterials = { ...player.resources.specialMaterials };
                  updatedSpecialMaterials[materialName] = (updatedSpecialMaterials[materialName] || 0) - amount;
        ...
        if (result) {
          const materialPrice = materialType === 'raw' ? 1 : 2;
          const goldGained = amount * materialPrice;
          console.log(`Du hast ${amount} ${materialName} für ${goldGained} Gold verkauft.`);
        } else {
          console.log('Die Aktion konnte nicht ausgeführt werden.');
        }
  ...
  private showGatherMaterialsAction(): void {
    ...
    console.log('\n=== MATERIAL GEWINNEN ===');
    console.log('Du kannst Arbeitskraft einsetzen, um Rohmaterialien zu gewinnen.');
    ...
    if (activeProperties.length === 0) {
      console.log('Du hast keine aktiven Besitztümer, von denen du sammeln kannst.');
      this.waitForEnter(() => {
        this.showActionMenu();
      });
      return;
    ...
    this.rl.question('\nVon welchem Besitztum möchtest du sammeln? (Nummer, 0 zum Abbrechen): ', (choiceStr) => {
      const choice = parseInt(choiceStr, 10);

      // Validate input
      if (isNaN(choice) || choice < 0 || choice > activeProperties.length) {
      ...
      this.rl.question(`Wie viel Arbeitskraft möchtest du einsetzen? (Max: ${player.resources.laborPower}): `, (laborStr) => {
        const laborAmount = parseInt(laborStr, 10);

        // Validate input
        if (isNaN(laborAmount) || laborAmount <= 0) {
          console.log('Ungültige Eingabe. Bitte gib eine positive Zahl ein.');
          this.waitForEnter(() => {
            this.showGatherMaterialsAction();
          });
        ...
        const result = this.engine.executeCommand({
          ...
          validate: (state) => {
            return state.phase === 'action' &&
                   state.actionPointsRemaining > 0 &&
                   state.players[state.currentPlayerId].resources.laborPower >= laborAmount;
          },
          execute: (state) => {
            // Roll for success
            ...
            const baseAmount = Math.floor(laborAmount * (success ? 1.0 : 0.5));
            ...
            return [{
              ...
              apply: (state) => {
                ...
                for (const [material, amount] of Object.entries(materialsGained)) {
        ...
        if (result) {
          ...
          for (const [material, amount] of Object.entries(event.rawMaterials)) {
            if (player.resources.rawMaterials[material] !== amount) {
              const difference = (amount || 0) - (player.resources.rawMaterials[material] || 0);
              if (difference > 0) {
  ...
  private showAcquirePropertyAction(): void {
    ...
    console.log('\n=== POSTEN ERWERBEN ===');
    console.log('Du kannst neue Besitztümer erwerben, wenn du genug Gold hast.');
    console.log(`Aktuelles Gold: ${player.resources.gold}`);
    ...
    this.rl.question('\nWelches Besitztum möchtest du erwerben? (1-6, 0 zum Abbrechen): ', (choiceStr) => {
      const choice = parseInt(choiceStr, 10);
      ...
      if (player.resources.gold < cost) {
        console.log(`Du hast nicht genug Gold für dieses Besitztum. Du benötigst ${cost} Gold.`);
        this.waitForEnter(() => {
          this.showAcquirePropertyAction();
        });
        return;
      ...
      const result = this.engine.executeCommand({
        ...
        execute: (state) => {
          ...
          const newPropertyId = Date.now().toString(36) + Math.random().toString(36).substr(2, 5);
  ...
  private showBuildFacilityAction(): void {
    ...
    if (!state.facilityBuildActionAvailable) {
      console.log('Du hast in dieser Runde bereits eine Einrichtung ausgebaut.');
      this.waitForEnter(() => {
        this.showActionMenu();
      });
      return;
    ...
    if (activeProperties.length === 0) {
      console.log('Du hast keine aktiven Besitztümer, auf denen du ausbauen kannst.');
      this.waitForEnter(() => {
        this.showActionMenu();
      });
      return;
    ...
    // Let user choose a property
    this.rl.question('\nAuf welchem Besitztum möchtest du ausbauen? (Nummer, 0 zum Abbrechen): ', (choiceStr) => {
      const choice = parseInt(choiceStr, 10);
      ...
      console.log('4. Einfache Verteidigungsanlage (3 Gold, 3 Holz, 1 Arbeitskraft)');
      console.log('5. Großer Oktaden-Schrein (8 Gold)');
      console.log('6. Marktplatz (16 Gold)');

      this.rl.question('\nWelche Einrichtung möchtest du bauen? (1-6, 0 zum Abbrechen): ', (facilityChoiceStr) => {
        const facilityChoice = parseInt(facilityChoiceStr, 10);

        // Validate input
        if (isNaN(facilityChoice) || facilityChoice < 0 || facilityChoice > 6) {
          console.log('Ungültige Eingabe.');
          this.waitForEnter(() => {
            this.showBuildFacilityAction();
          });
        ...
        if (facilityConfig.buildRequirements.propertyTypes &&
            !facilityConfig.buildRequirements.propertyTypes.includes(selectedProperty.type)) {
          console.log(`Diese Einrichtung kann nicht auf einem ${selectedProperty.type} gebaut werden.`);
          this.waitForEnter(() => {
            this.showBuildFacilityAction();
          });
          return;
        ...
        if (resources.gold && player.resources.gold < resources.gold) {
          canBuild = false;
          missingResources.push(`${resources.gold - player.resources.gold} Gold`);
        }

        if (resources.laborPower && player.resources.laborPower < resources.laborPower) {
          canBuild = false;
          missingResources.push(`${resources.laborPower - player.resources.laborPower} Arbeitskraft`);
        }
        ...
        if (resources.rawMaterials) {
          for (const [material, amount] of Object.entries(resources.rawMaterials)) {
            ...
            if (playerAmount < amountNumber) {
              canBuild = false;
              missingResources.push(`${amountNumber - playerAmount} ${material}`);
            }
          }
        }

        if (!canBuild) {
          console.log(`Du hast nicht genug Ressourcen für diese Einrichtung. Es fehlt: ${missingResources.join(', ')}`);
          this.waitForEnter(() => {
            this.showBuildFacilityAction();
          });
          return;
        ...
        const result = this.engine.executeCommand({
          ...
          validate: (state) => {
            return state.phase === 'action' && state.facilityBuildActionAvailable;
          },
          execute: (state) => {
            // Create facility instance
            const newFacilityId = Date.now().toString(36) + Math.random().toString(36).substr(2, 5);
            ...
            return [{
              ...
              apply: (state) => {
                const player = { ...state.players[state.currentPlayerId] };
                const property = { ...state.properties[selectedProperty.id] };

                // Update player resources
                player.resources = {
                  ...player.resources,
                  gold: player.resources.gold - (resources.gold || 0),
                  laborPower: player.resources.laborPower - (resources.laborPower || 0)
                };
                ...
                if (resources.rawMaterials) {
                  ...
                  for (const [material, amount] of Object.entries(resources.rawMaterials)) {
                    ...
                    const amountNumber = amount as number;
                    updatedRawMaterials[materialKey] = (updatedRawMaterials[materialKey] || 0) - amountNumber;
                  }
        ...
        if (result) {
          console.log(`Du hast eine neue Einrichtung gebaut: ${facilityConfig.name}`);
        } else {
          console.log('Die Aktion konnte nicht ausgeführt werden.');
        }
