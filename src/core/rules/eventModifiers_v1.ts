...
export function activeEventsForRound(events: GlobalEventState[], round: number): GlobalEventState[] {
  return events.filter((e) => round >= e.startsAtRound && round <= e.endsAtRound);
}

export function isDeneraRiotTriggered(events: GlobalEventState[], round: number): boolean {
  for (const e of activeEventsForRound(events, round)) {
    if (e.tableRollTotal === 27) return true;
    const triggered = (e.meta as any)?.deneraRiotTriggered;
    if (triggered === true) return true;
  }
  return false;
}

export function workshopUpkeepMods(events: GlobalEventState[], round: number): WorkshopUpkeepMods {
  const mods: WorkshopUpkeepMods = { laborFlat: 0, goldFlat: 0, goldPerTier: 0 };
  for (const e of activeEventsForRound(events, round)) {
  ...
  // Denera riot implied by other events
  if (!activeEventsForRound(events, round).some((e) => e.tableRollTotal === 27) && isDeneraRiotTriggered(events, round)) {
    mods.goldPerTier += 1;
  }
  return mods;
}

export function officeIncomeMods(events: GlobalEventState[], round: number): OfficeIncomeMods {
...
export function taxGoldPerRound(events: GlobalEventState[], round: number): number {
  let tax = 0;
  for (const e of activeEventsForRound(events, round)) {
...
export function moneyActionMods(events: GlobalEventState[], round: number): MoneyActionMods {
  const mods: MoneyActionMods = { lendDc: 0, sellDc: 0, influenceDc: 0, bonusGoldPerTwoInvestments: 0 };
...
export function rawAutoConvertDivisor(materialId: string, events: GlobalEventState[], round: number): number {
...
export function marketDeltaPerInvestment(materialId: string, events: GlobalEventState[], round: number): number {
  const material = getMaterialOrThrow(materialId);
  let delta = 0;
  ...
  for (const e of activeEventsForRound(events, round)) {
    ...
    switch (e.tableRollTotal) {
      case 10: {
        ...
        if (material.tier === 'expensive') add(meta.marketExpensiveAllPenalty, -1);
        break;
      }
      case 11: {
      ...
      case 17: {
        ...
        if (material.kind === 'special' && material.tier === 'expensive') delta += 2;
        break;
      }
      case 19: {
        // Alle teuren SM -1d6 Gold Marktwert.
        if (material.kind === 'special' && material.tier === 'expensive') add(meta.marketExpensiveSpecialPenalty, -1);
        break;
      }
      case 21: {
        // Metallische Rohstoffe -1d4 Gold Marktwert.
        if (material.kind === 'raw' && material.tags.includes('metal')) add(meta.marketMetalPenalty, -1);
        break;
      }
      case 33: {
      ...
      case 35: {
        ...
        if (material.kind === 'special' && material.tier === 'expensive') add(meta.marketExpensiveSpecialBonus, 1);
        break;
      }
      case 40: {
...
export function saleBonusGoldForAction(
  ...
  const any = (pred: (m: ReturnType<typeof getMaterialOrThrow>) => boolean) => sold.some(pred);
    ...
    switch (e.tableRollTotal) {
      ...
      case 14:
        if (any((m) => m.tags.includes('magic'))) {
          const roll = (meta.marketMagicComponentsSaleBonus as any)?.total;
          if (typeof roll === 'number') bonus += roll;
        }
        break;
      case 20:
      ...
      case 9:
        if (any((m) => m.tags.includes('building'))) {
          const hasCheapOrBasic = any((m) => m.tags.includes('building') && m.tier !== 'expensive');
          const hasExpensive = any((m) => m.tags.includes('building') && m.tier === 'expensive');
          ...
          if (hasCheapOrBasic && typeof cheapBasic === 'number') bonus += cheapBasic;
          if (hasExpensive && typeof expensive === 'number') bonus += expensive;
        }
        break;
      case 15:
        if (soldMaterialIds.includes('raw.wood') || soldMaterialIds.includes('raw.fish')) {
