...
type AgentOutcome =
  PlaytestReport['outcomes']['byAgent'][keyof PlaytestReport['outcomes']['byAgent']];
...
function sectionForReport(report: PlaytestReport, label: string): string {
  ...
  lines.push(`## ${label}`);
  lines.push(
    `Config: runs=${report.config.runs}, rounds=${report.config.rounds}, seed=${report.config.seed}..${report.config.seed + report.config.runs - 1}`
  );
  lines.push('');
  lines.push(
  ...
  const totalTeDamaged = sumByAgent(
    report,
    (a) => a.sell.tradeEnterpriseDamagedEvents
  );
  const totalTeLost = sumByAgent(
    report,
    (a) => a.sell.tradeEnterpriseLostEvents
  );
...
async function main() {
  ...
  if (paths.length === 0) {
    console.error(
      'Usage: bun src/playtest/analyzeBatch.ts <report.json>... [--out <file.md>]'
    );
    process.exitCode = 1;
    return;
  }
  ...
  lines.push('');
  lines.push(
    'Diese Auswertung nutzt den Playtest-Runner (Planner-Agenten, keine LLM-Calls) und bewertet den Erfolg über GoldEq-Score.'
  );
  lines.push('');
  ...
  const nameByAgentId = Object.fromEntries(
    reports.flatMap((r) =>
      r.scenario.players.map((p) => [p.agentId, p.displayName] as const)
    )
  );
  ...
  const headers = [
    'Strategie',
    ...horizons.flatMap((h) => [`${h}R Score`, `${h}R win`]),
  ];
