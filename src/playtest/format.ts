...
export function formatPlaytestMarkdown(report: PlaytestReport): string {
  ...
  const byWinRate = [...agents].sort(
    (a, b) => b[1].winRateScore - a[1].winRateScore
  );
  ...
  lines.push(
    '- Mode: `verpachtet` = Gold+Einfluss+AK, `Eigenproduktion` = AK (mit Gold-Unterhalt), öffnet Werkstatt/Lager im Stadtbesitz.'
  );
  for (const [agentId, a] of byWinRate) {
  ...
  lines.push(
    `- Markt-Rohmaterial: samples=${report.systems.market.raw.samples}`
  );
  lines.push(
  ...
  lines.push(
    `- Markt-Sondermaterial: samples=${report.systems.market.special.samples}`
  );
  lines.push(
  ...
  lines.push(
    '- Agenten verbessern: Zielgerichteter Marktverkauf (Trade-Märkte), gezielte Pächter/Anhänger-Rekrutierung, Truppen-/Unterwelt-Synergien.'
  );
  lines.push(
    '- Ereignisabdeckung erweitern (z.B. Amts-spezifische Effekte, Domänen-Angriffe, Provinzinspektion) – oder diese explizit als „nicht modelliert“ markieren.'
  );
  lines.push(
    '- Facility-Katalog (Einrichtungen der Domänen/Stadt/Circel/Ämter/Handel) als Datenmodell abbilden und Ertrags-/Upkeep-Effekte in Maintenance verdrahten.'
  );
  lines.push(
    '- Politische Schritte (Aktion 5) später implementieren, sobald Kernökonomie stabil ist.'
  );
  lines.push('');
